...
function initPWA(){
  const buttons = [...document.querySelectorAll('#installAppBtn, #installAppBtnTop, .floating-install')];
  const btn = buttons[0];
  ...
  window.addEventListener('beforeinstallprompt', event => {
    ...
    buttons.forEach(b => b.classList.add('is-ready'));
  });

  window.addEventListener('appinstalled', () => {
    deferredInstallPrompt = null;
    buttons.forEach(b => b.classList.add('is-installed'));
  });
...
function setupTopLinks(){
  const wa = document.querySelectorAll('[data-whatsapp-link], .whatsapp');
  const phone = String(CONFIG.WHATSAPP_NUMBER || '').replace(/[^0-9]/g,'') || '918921696649';
  wa.forEach(a => a.href = `https://wa.me/${phone}`);
  document.querySelectorAll('[data-shopping-link], #shoppingMenuLink').forEach(a => {
    a.href = CONFIG.SHOPPING_URL || 'https://www.freshly-online.com/freshlymart/#wellness';
    a.target = '_blank';
    a.rel = 'noopener';
  });
}
...
async function loadMenu(){
  const menu = document.querySelector('.menu');
  if(!menu) return;
  const fallback = fallbackMenuRows();
  let rows = await loadRowsFromBackend('menu', fallback);
  rows = rows.filter(row => (getVal(row,'Status') || 'Active').toLowerCase() === 'active')
             .filter(row => !['admin','install app'].includes(String(getVal(row,'MenuName','Name','Title') || '').trim().toLowerCase()))
             .sort((a,b) => {
                const an = String(getVal(a,'MenuName','Name','Title') || '').trim().toLowerCase();
                const bn = String(getVal(b,'MenuName','Name','Title') || '').trim().toLowerCase();
                const as = an === 'shopping' ? 9999 : Number(getVal(a,'SortOrder') || 999);
                const bs = bn === 'shopping' ? 9999 : Number(getVal(b,'SortOrder') || 999);
                return as - bs;
             });
  if(!rows.length) rows = fallback;
  menu.innerHTML = renderMenuRows(rows);
  setupTopLinks();
}

function fallbackMenuRows(){
  return [
    {MenuID:'MENU001',ParentID:'',MenuName:'Home',PageLink:'index.html',SortOrder:'1',Status:'Active',OpenType:'Same'},
    {MenuID:'MENU002',ParentID:'',MenuName:'Online Programs',PageLink:'#',SortOrder:'2',Status:'Active',OpenType:'Same'},
    {MenuID:'MENU003',ParentID:'MENU002',MenuName:'Online Group Yoga',PageLink:'online-group-yoga.html',SortOrder:'1',Status:'Active',OpenType:'Same'},
    {MenuID:'MENU004',ParentID:'MENU002',MenuName:'Corporate Chair Yoga',PageLink:'corporate-chair-yoga.html',SortOrder:'2',Status:'Active',OpenType:'Same'},
    {MenuID:'MENU005',ParentID:'MENU002',MenuName:'Therapeutic Yoga',PageLink:'therapeutic-yoga.html',SortOrder:'3',Status:'Active',OpenType:'Same'},
    {MenuID:'MENU006',ParentID:'MENU002',MenuName:'Online Karate',PageLink:'online-karate.html',SortOrder:'4',Status:'Active',OpenType:'Same'},
    {MenuID:'MENU007',ParentID:'MENU002',MenuName:'Online Dance',PageLink:'online-dance.html',SortOrder:'5',Status:'Active',OpenType:'Same'},
    {MenuID:'MENU008',ParentID:'MENU002',MenuName:'Online Music',PageLink:'online-music.html',SortOrder:'6',Status:'Active',OpenType:'Same'},
    {MenuID:'MENU009',ParentID:'MENU002',MenuName:'Online Drawing',PageLink:'online-drawing.html',SortOrder:'7',Status:'Active',OpenType:'Same'},
    {MenuID:'MENU010',ParentID:'',MenuName:'Offline Programs',PageLink:'#',SortOrder:'3',Status:'Active',OpenType:'Same'},
    {MenuID:'MENU011',ParentID:'MENU010',MenuName:'Yoga',PageLink:'offline-classes.html#yoga',SortOrder:'1',Status:'Active',OpenType:'Same'},
    {MenuID:'MENU012',ParentID:'MENU010',MenuName:'Karate',PageLink:'offline-classes.html#karate',SortOrder:'2',Status:'Active',OpenType:'Same'},
    {MenuID:'MENU013',ParentID:'MENU010',MenuName:'Dance',PageLink:'offline-classes.html#dance',SortOrder:'3',Status:'Active',OpenType:'Same'},
    {MenuID:'MENU014',ParentID:'MENU010',MenuName:'Music',PageLink:'offline-classes.html#music',SortOrder:'4',Status:'Active',OpenType:'Same'},
    {MenuID:'MENU015',ParentID:'MENU010',MenuName:'Drawing',PageLink:'offline-classes.html#drawing',SortOrder:'5',Status:'Active',OpenType:'Same'},
    {MenuID:'MENU017',ParentID:'',MenuName:'Packages',PageLink:'packages.html',SortOrder:'4',Status:'Active',OpenType:'Same'},
    {MenuID:'MENU018',ParentID:'',MenuName:'Register',PageLink:'register.html',SortOrder:'5',Status:'Active',OpenType:'Same'},
    {MenuID:'MENU019',ParentID:'',MenuName:'Teach With Us',PageLink:'become-a-teacher.html',SortOrder:'6',Status:'Active',OpenType:'Same'},
    {MenuID:'MENU016',ParentID:'',MenuName:'Shopping',PageLink:'shopping',SortOrder:'99',Status:'Active',OpenType:'New'}
  ];
}

function menuLink(row){
  const raw = getVal(row,'PageLink','Link','URL') || '#';
  if(raw.toLowerCase() === 'shopping') return CONFIG.SHOPPING_URL || 'https://www.freshly-online.com/freshlymart/#wellness';
  if(raw.toLowerCase() === 'whatsapp') return `https://wa.me/${String(CONFIG.WHATSAPP_NUMBER || '').replace(/[^0-9]/g,'')}`;
  return raw;
}
...
function renderMenuRows(rows){
  ...
  rows.forEach(row => {
    const parent = getVal(row,'ParentID','ParentId') || '';
    if(!byParent.has(parent)) byParent.set(parent, []);
    byParent.get(parent).push(row);
  });
  const roots = byParent.get('') || [];
  const attrs = row => {
    const open = (getVal(row,'OpenType') || 'Same').toLowerCase();
    return open === 'new' ? ' target="_blank" rel="noopener"' : '';
  };
  return roots.map(row => {
    const id = getVal(row,'MenuID','Id');
    const children = (byParent.get(id) || []).sort((a,b)=>Number(getVal(a,'SortOrder')||999)-Number(getVal(b,'SortOrder')||999));
    const name = getVal(row,'MenuName','Name','Title') || 'Menu';
    const link = menuLink(row);
    if(children.length){
      return `<div class="dropdown"><button class="dropbtn" type="button">${escapeHTML(name)}</button><div class="dropdown-content">${children.map(child => `<a href="${escapeHTML(menuLink(child))}"${attrs(child)}>${escapeHTML(getVal(child,'MenuName','Name','Title'))}</a>`).join('')}</div></div>`;
    }
    return `<a href="${escapeHTML(link)}"${attrs(row)}>${escapeHTML(name)}</a>`;
  }).join('');
}
