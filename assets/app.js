...
function initMobileMenu(){
  ...
  menu.addEventListener('click', event => {
    ...
    const dropdownButton = event.target.closest('.dropbtn');
    if(dropdownButton){
    ...
    const subdropLink = event.target.closest('.subdrop > a');
    if(subdropLink && subdropLink.nextElementSibling && subdropLink.nextElementSibling.classList.contains('subdrop-content')){
      event.preventDefault();
      const subdrop = subdropLink.closest('.subdrop');
      if(subdrop) subdrop.classList.toggle('open');
      return;
    }
    ...
    if(link){
      ...
      menu.querySelectorAll('.open').forEach(el => el.classList.remove('open'));
    }
  });
}
...
function updateSideActionLinks(){
  ...
  // Keep every Shopping menu/button pointed to the Freshly Mart wellness link from config.
  document.querySelectorAll('a').forEach(a => {
    const text = (a.textContent || '').trim().toLowerCase();
    if(text === 'shopping' || text.includes('wellness products')){
...
function menuAnchorHTML(name, link, row){
  if(String(name || '').trim().toLowerCase() === 'shopping'){
    link = CONFIG.SHOPPING_URL || link || 'https://www.freshly-online.com/freshlymart/#wellness';
  }
