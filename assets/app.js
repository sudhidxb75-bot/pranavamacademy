const CONFIG = {
  // Paste your published package CSV URL here. Keep this internal; it is not shown on the website.
...
function hasPackageName(p){
  return !!getVal(p,'Program','PackageName','Name','ClassName');
}
...
function setupFilters(data){
  ...
  bar.innerHTML=cats.map((c,i)=>`<button class="filter-btn ${i===0?'active':''}" data-filter="${c}">${c}</button>`).join('');
...
function handleForms(){
 document.querySelectorAll('form[data-sheet]').forEach(form=>form.addEventListener('submit',async e=>{
  ...
  if(!CONFIG.APPS_SCRIPT_URL || CONFIG.APPS_SCRIPT_URL.includes('PASTE_')){ if(msg) msg.textContent='Form is ready. Please connect the backend URL in assets/app.js.'; return; }
