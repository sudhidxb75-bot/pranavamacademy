...
function doGet(e) {
  ...
  try {
    ...
    if (action === 'menu') {
      result = { ok: true, status: 'success', data: getActiveSheetObjects_('Menu') };
      return webOutput_(result, params);
    }

    if (action === 'adminLogin') {
      const password = String(params.password || '');
      result = { ok: true, authenticated: password === getSetting_('AdminPassword', '12345') };
      return webOutput_(result, params);
    }
    ...
    if (action === 'adminAction') {
      result = runAdminAction_(String(params.task || ''));
      return webOutput_(result, params);
    }
    ...
    result = {
      ...
      actions: ['siteImages', 'sliderBanners', 'packages', 'menu', 'adminData', 'setup']
    };
    return webOutput_(result, params);
  } catch (err) {
...
function headerMap_() {
  return {
    ...
    Menu: ['MenuID','ParentID','MenuName','PageLink','SortOrder','Status','OpenType','Notes'],
...
function seedSettings_() {
  ...
  const existing = getSheetObjects_('Settings').map(r => String(r.Key || '').trim());
  const rows = [
    ['AdminPassword','12345','Change this password after first setup'],
    ['WhatsAppNumber','918921696649','Used for website WhatsApp button'],
    ['ShoppingURL','https://www.freshly-online.com/freshlymart/#wellness','Shopping menu link for Freshly Mart wellness products']
  ].filter(r => existing.indexOf(r[0]) === -1);
...
function getSetting_(key, fallback) {
  ...
  return found && String(found.Value || '').trim() ? String(found.Value).trim() : fallback;
}

function defaultMenu_() {
  return [
    ['MENU001','','Home','index.html',1,'Active','Same',''],
    ['MENU002','','Online Programs','#',2,'Active','Same',''],
    ['MENU003','MENU002','Online Yoga','yoga.html',1,'Active','Same',''],
    ['MENU004','MENU002','Online Group Yoga','online-group-yoga.html',2,'Active','Same',''],
    ['MENU005','MENU002','Corporate Chair Yoga','corporate-chair-yoga.html',3,'Active','Same',''],
    ['MENU006','MENU002','Therapeutic Yoga','therapeutic-yoga.html',4,'Active','Same',''],
    ['MENU007','MENU002','Online Karate','online-karate.html',5,'Active','Same',''],
    ['MENU008','MENU002','Online Dance','online-dance.html',6,'Active','Same',''],
    ['MENU009','MENU002','Online Music','online-music.html',7,'Active','Same',''],
    ['MENU010','MENU002','Online Drawing','online-drawing.html',8,'Active','Same',''],
    ['MENU011','','Offline Programs','#',3,'Active','Same',''],
    ['MENU012','MENU011','Yoga','offline-classes.html#yoga',1,'Active','Same',''],
    ['MENU013','MENU011','Karate','offline-classes.html#karate',2,'Active','Same',''],
    ['MENU014','MENU011','Dance','offline-classes.html#dance',3,'Active','Same',''],
    ['MENU015','MENU011','Music','offline-classes.html#music',4,'Active','Same',''],
    ['MENU016','MENU011','Drawing','offline-classes.html#drawing',5,'Active','Same',''],
    ['MENU017','','Packages','packages.html',4,'Active','Same',''],
    ['MENU018','','Register','register.html',5,'Active','Same',''],
    ['MENU019','','Teach With Us','become-a-teacher.html',6,'Active','Same',''],
    ['MENU020','','Shopping','https://www.freshly-online.com/freshlymart/#wellness',99,'Active','New','Freshly Mart wellness products']
  ];
}

function getAdminData_() {
  setupPranavamAcademySheets();
  const registrations = getSheetObjects_('Registrations').slice(-50).reverse();
  const corporateLeads = getSheetObjects_('CorporateLeads').slice(-50).reverse();
  const teacherApplications = getSheetObjects_('TeacherApplications').slice(-50).reverse();
  ...
  const menuRows = getActiveSheetObjects_('Menu');
  const payments = getSheetObjects_('Payments');
  return {
    stats: {
      Registrations: getSheetObjects_('Registrations').length,
      CorporateLeads: getSheetObjects_('CorporateLeads').length,
      CorporateRegistrations: getSheetObjects_('CorporateRegistrations').length,
      TeacherApplications: getSheetObjects_('TeacherApplications').length,
      ActivePackages: packagesRows.length,
      ActiveBanners: banners.length,
      ActiveImages: images.length,
      ActiveMenuItems: menuRows.length,
      Payments: payments.length
    },
    registrations: registrations,
    corporateLeads: corporateLeads,
...
function runAdminAction_(task) {
  setupPranavamAcademySheets();
  if (task === 'createReports') {
    createAllReports(false);
    return {ok:true, status:'success', message:'Reports and print sheets created / refreshed.'};
  }
  if (task === 'backupData') {
    const name = backupMainData_();
    return {ok:true, status:'success', message:'Backup created: ' + name};
  }
  if (task === 'clearGenerated') {
    const count = deleteGeneratedSheets_(name => isGeneratedReportSheet_(name) || name.indexOf('DailySheet_') === 0 || name.indexOf('Backup_') === 0);
    return {ok:true, status:'success', message: count + ' generated sheet(s) cleared. Main data was not deleted.'};
  }
  return {ok:false, status:'error', message:'Unknown admin action.'};
}

function backupMainData_() {
  const ss = getSpreadsheet_();
  const stamp = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyyMMdd_HHmmss');
  const backupSheetName = 'Backup_' + stamp;
  const out = ss.insertSheet(backupSheetName);
  let row = 1;
  ...
  sheets.forEach(name => {
    ...
    out.getRange(row,1).setValue(name).setFontWeight('bold').setBackground('#0f5132').setFontColor('#ffffff');
    row++;
    if (sh && sh.getLastRow() && sh.getLastColumn()) {
      const values = sh.getDataRange().getDisplayValues();
      out.getRange(row,1,values.length,values[0].length).setValues(values);
      row += values.length + 2;
    } else {
      out.getRange(row,1).setValue('No data');
      row += 2;
    }
  });
  out.autoResizeColumns(1, Math.min(12, out.getLastColumn() || 1));
...
function createDashboardReport(showAlert) {
  ...
  const rows = [
    ...
    ['Active Menu Items', menuRows.length, 'Status = Active']
  ];
